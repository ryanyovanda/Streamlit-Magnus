...
if uploaded_csv:
    ...
    if page == "Dashboard":
        ...
        with col1:
            ...
            fig_lr = go.Figure()
            for band in band_cols:
                left_val = abs_df.loc[abs_df['Band'] == band.capitalize(), region_map["Left"]].mean(axis=1).values[0]
                right_val = abs_df.loc[abs_df['Band'] == band.capitalize(), region_map["Right"]].mean(axis=1).values[0]
        ...
        with col2:
            ...
            fig_fb = go.Figure()
            for band in band_cols:
                front_val = abs_df.loc[abs_df['Band'] == band.capitalize(), region_map["Front"]].mean(axis=1).values[0]
                back_val = abs_df.loc[abs_df['Band'] == band.capitalize(), region_map["Back"]].mean(axis=1).values[0]
        ...
        for i, band in enumerate(band_cols):
            with cols[i]:
                ...
                left = abs_df.loc[i, region_map["Left"]].mean()
                right = abs_df.loc[i, region_map["Right"]].mean()
